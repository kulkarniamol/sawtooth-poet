...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.find('/attestation/v3/sigrl') == 0:
            self._get_sig_rl()
        else:
            self._respond(404)
    ...
    def do_POST(self):
        # pylint: disable=invalid-name
        if self.path.find('/attestation/v3/report') == 0:
    ...
    def __init__(self, proxy_config):
        ...
        self.client = \
            ias_client.IasClient(
                ...
                ias_subscription_key=proxy_config['ias_subscription_key'])
...
def get_server():
    ...
    valid_keys = set(['proxy_name', 'proxy_port', 'ias_url', 'ias_subscription_key'])
